...
// FIX: Define DoubtListItem outside the component to prevent re-creation on render and fix key prop type error.
type DoubtListItemProps = {
  doubt: Doubt;
};

const DoubtListItem = ({ doubt }: DoubtListItemProps) => (
  <Link to={`/doubt/${doubt.id}`} className="block p-4 border-b dark:border-gray-700 last:border-b-0 hover:bg-slate-50 dark:hover:bg-gray-700/50 transition-colors">
      <div className="flex justify-between items-start">
          <p className="font-semibold text-blue-600 dark:text-blue-400 group-hover:underline line-clamp-1">{doubt.title}</p>
          {doubt.isResolved && <span className="text-xs font-bold bg-green-100 text-green-800 dark:bg-green-900/50 dark:text-green-300 px-2 py-1 rounded-full flex-shrink-0 ml-2">Resolved</span>}
      </div>
...
const UserProfilePage = () => {
  ...
  const myDoubts = useMemo(() => {
    if (!user) return [];
    return doubts.filter(d => d.authorId === user.id).sort((a,b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
  }, [doubts, user]);
  ...
  const answeredDoubts = useMemo(() => {
      return doubts.filter(d => answeredDoubtIds.has(d.id)).sort((a,b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md border border-slate-200 dark:border-gray-700 p-6 md:p-8 mb-8 flex flex-col md:flex-row items-center gap-6 md:gap-8">
        ...
        <div className="flex-1 text-center md:text-left">
          ...
          <div className="inline-flex items-center gap-1.5 mt-2">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-orange-500" viewBox="0 0 20 20" fill="currentColor">
                  <path fillRule="evenodd" d="M4 4a2 2 0 012-2h8a2 2 0 012 2v12a1 1 0 110 2h-3v-2a1 1 0 00-1-1H9a1 1 0 00-1 1v2H5a1 1 0 110-2V4zm3 1a1 1 0 011-1h2a1 1 0 011 1v2a1 1 0 01-1 1H8a1 1 0 01-1-1V5zm1 5a1 1 0 00-1 1v2a1 1 0 001 1h2a1 1 0 001-1v-2a1 1 0 00-1-1H8z" clipRule="evenodd" />
              </svg>
              <span className="font-medium text-gray-700 dark:text-gray-300">{user.branch}</span>
          </div>
      ...
      {stats && (
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md border border-slate-200 dark:border-gray-700 p-6 mb-8">
